...
#import "CIMProgressHUD.h"

@implementation CIMProgressHUD

@synthesize interaction, window, background, hud, cimIndicatorView, image, label;

+ (CIMProgressHUD *)shared {
    static dispatch_once_t onceToken = 0;
    static CIMProgressHUD * progressHUD;
    
    
    dispatch_once(&onceToken, ^{
        progressHUD = [[CIMProgressHUD alloc]init];
    });
...
- (id)init {
    ...
    background = nil; hud = nil; image = nil; label = nil; cimIndicatorView=nil;
    self.alpha = 0;
    return self;
}
...
- (void)hudMake:(NSString *)status image:(UIImage *)img spin:(BOOL)spin hide:(BOOL)hide {
    ...
    //---------------------------------------------------------------------------------------------------------------------------------------------
    if (cimIndicatorView) [cimIndicatorView startAnimation]; else [cimIndicatorView stopAnimation];
    
    cimIndicatorView.hidden = !image.hidden;
    //---------------------------------------------------------------------------------------------------------------------------------------------
    [self hudSize];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    //---------------------------------------------------------------------------------------------------------------------------------------------
    if (cimIndicatorView == nil)
    {
        cimIndicatorView = [[CIMIndicatorView alloc]init];
    }
    if (cimIndicatorView.superview == nil) {
        [hud addSubview:cimIndicatorView];
    }
    //---------------------------------------------------------------------------------------------------------------------------------------------
    if (image == nil)
    {
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    [cimIndicatorView removeFromSuperview]; cimIndicatorView=nil;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    CGFloat hudWidth = 90, hudHeight = 90;
    //---------------------------------------------------------------------------------------------------------------------------------------------
    if (label.text != nil)
    {
...
//        NSLog(@"%@", NSStringFromCGRect(label.frame));
    ...
    image.center = cimIndicatorView.center =CGPointMake(imagex, imagey);
    //---------------------------------------------------------------------------------------------------------------------------------------------
